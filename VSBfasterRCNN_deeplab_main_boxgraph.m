function VSBfasterRCNN_deeplab_main_boxgraph(eidx)
% The main function for tracking by motion segmentation
clear EXP;

%% parameters init.
EXP = VSBfasterRCNN_deeplab_exp_detail(eidx);
...
for s = 1:num_seq
    ...
    if EXP.add_box == 1 % include bbox into the model
        [EXP, costs_box_vertices, box_edges, cost_box_edges, boxes] = VSBfasterRCNN_deeplab_read_box(EXP,s);
    ...
    problem_file = [EXP.output_dir  s_name(1:end-1) '_problem.h5'];
...
%         marray_save(problem_file, dataset_name, sigmas, write_mode);
        segments = VSBfasterRCNN_deeplab_convertSEGforCpp(EXP,boxes);
        write_mode = 'append';
