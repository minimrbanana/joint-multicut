function EXP = VSBfasterRCNN_deeplab_compute_box_unary_cost(EXP,s_idx,box_color_dir,box_flow_dir,box_color_seg_dir,box_flow_seg_dir)
%
...
try 
    ...
    if EXP.isDeepMask == 0
        checkfolder = ['VSBFRCNN_' T_label 'score_' int2str(EXP.box_score_min) '_rank_' int2str(EXP.box_rank_num) '_nms_' num2str(EXP.box_NMS) '/'];
    else
        checkfolder = ['VSBFRCNN_' T_label 'score_' int2str(EXP.box_score_min) '_rank_' int2str(EXP.box_rank_num) '_DeepMask_1/'];
    end
    checkpath = [checkdir checkfolder];
    try
        load([checkpath EXP.label(s_idx).name '/costs_box_vertices.mat']);
        save([EXP.output_dir 'costs_box_vertices.mat'], 'num_node','node2frame','frame2node','U','U_cost','U_list','U_lsda_score','F7','Segment','-v7.3');
    catch
        if ~exist([checkpath EXP.label(s_idx).name],'dir')
            mkdir([checkpath EXP.label(s_idx).name]);
        end
        %% detection dir
        ...
        for i = 1:num_frame
            ...
            if ~isempty(box)
                if EXP.isDeepMask == 1
                    ...
                    load(['/BS/joint-multicut/work/DetectionRes/F' stemp '_DeepMask_S/' EXP.label(s_idx).name '/' EXP.label{s_idx} stemp2 '.mat']);
                    seg = segment;
                end
        ...
        save([checkpath EXP.label(s_idx).name '/costs_box_vertices.mat'], 'num_node','node2frame','frame2node','U','U_cost','U_list','U_lsda_score','F7','Segment','-v7.3');
        save([EXP.output_dir 'costs_box_vertices.mat'], 'num_node','node2frame','frame2node','U','U_cost','U_list','U_lsda_score','F7','Segment','-v7.3');
    end
end
%% output
...
%
im_dir = [EXP.im_dir  EXP.label(s_idx).name ];
